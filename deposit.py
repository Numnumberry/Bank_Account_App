...
def deposit(event, username, amount):
	...
	if (balance < 0):
		...
		t = classes.Transaction(username, 'D', float(amount), balance)
...
def doDeposit(event, username):
	...
	ws = depositFrame.winfo_screenwidth()
	hs = depositFrame.winfo_screenheight()

	x = (ws/2) - (w/2)
	y = (hs/2) - (h/2)

	depositFrame.geometry('%dx%d+%d+%d' % (w, h, x, y))
    
	amountLabel = Label(depositFrame, text = "Amount:")
	amountLabel.pack()

	a = Entry(depositFrame)
	a.pack()
	
	button = Button(depositFrame, text = "Deposit", height = 2, width = 20)
	button.bind('<Button-1>',lambda event: deposit(event, username, a.get()))
	button.pack()
